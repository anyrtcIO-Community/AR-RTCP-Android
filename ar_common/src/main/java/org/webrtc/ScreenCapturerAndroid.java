...
public class ScreenCapturerAndroid
  ...
  **/
  public ScreenCapturerAndroid(Intent mediaProjectionPermissionResultData,
                               MediaProjection.Callback mediaProjectionCallback, ScreenCallback callback) {
...
////    byte[] ret = new byte[(width * height * 3 / 2 * ( height + (height + 1) / 2))];
//    ByteBuffer buffer = ByteBuffer.wrap(ret);
//    surfaceTextureHelper.textureToYUV(buffer, width, height, width, oesTextureId, transformMatrix);
//    mScreenCallback.ScreenData(ret, width, height);
    //capturerObserver.onByteBufferFrameCaptured( new byte[buffer.capacity()], width,height, 0, timestampNs);
  }
